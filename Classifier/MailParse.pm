...
=item * Japanese text via a built-in EUC-JP character-class tokeniser
...
Converts an HTML color value (name, C<#rrggbb>, or C<#rgb> shorthand) into
    ...
    if ($htmlfontcolor ne $htmlbackcolor) {
        ...
        while ($p < length($bigline)) {
            ...
            if ($lang eq 'Nihongo') {
                ...
                while ($line =~
                    s/^$euc_jp*? ([A-Za-z][A-Za-z\']{2,44}|
                    $non_symbol_euc_jp{2,45})
                    (?:[_\-,\.\"\'\)\?!:;\/& \t\n\r]{0,5}|$)
                    //ox)
                {
                    if ((!$in_headers) && ($first20count < 20)) {
                        $first20count++;
                        $first20 .= " $1";
                    }
                    $self->update_word(
    ...
    (([\"\'])(.*?)\4|([^ \t>]+)($|([ \t>])))
        ...
        if (($attribute =~ /^href$/i) && ($tag =~ /^(a|link|base|area)$/i)) {
            ...
            if ($value =~ /^mailto:/i) {
                ...
                ([[:alpha:]0-9\-_\.]+?))
                ([>\&\?\:\/\" \t]|$)/ix)) {
                    $self->update_word($1, $encoded, 'mailto:',
                    ($3 ? '[\\\>\&\?\:\/]' : $end_quote), '');
                    $self->add_url($2, $encoded, '@',
                    ($3 ? '[\\\&\?\:\/]' : $end_quote), '');
                }
            } else {
...
method parse_html ($line, $encoded) {
    ...
    while ($line =~ s/(<\/?(?!(?:$spacing_tags|$non_spacing_tags)\W) [a-z0-9]+(?:\s+.*?)?\/?>)//iox) {
    ...
    while ($found && ($line ne '')) {
        ...
        if ($in_html_tag) {
            if ($line =~ s/^([^>]*?)>//) {
                ...
                $html_tag =~ s/=\n ?//g;
                $html_arg =~ s/=\n ?//g;
                $self->update_tag($html_tag, $html_arg, $html_end, $encoded);
                $html_tag = '';
                $html_arg = '';
                $found = 1;
                next;
...
method parse_file ($file, $max_size = undef, $reset = undef) {
    ...
    if (defined $mangle && $mangle->config()->get('auto_detect_language')) {
    ...
    while (<$msg>) {
        $size_read += length($_);
        $self->parse_line($_);
        if (($max_size > 0) && ($size_read > $max_size)) {
            last;
        }
    }
    close $msg;
...
method parse_line ($read) {
    if ($read ne '') {
        ...
        while ($read =~ s/(.*?)[\r\n]+//) {
            ...
            if (!$in_headers && ($cur_encoding =~ /quoted\-printable/i)) {
                if ($line =~ s/=\r\n$//) {;
            ...
            ($cur_encoding !~ /base64/i)) {
                ...
                $line = $self->parse_line_with_internal_parser($line);
            }
            if (defined($color_resolver)) {
                if (!$in_html_tag) {
                    $colorized .= $ut;
...
method clear_out_base64() {
    ...
    if ($base64 ne '') {
        ...
        if ($lang eq 'Nihongo') {
            ...
            $encoding_candidates{ $lang }->@*);
            $decoded = $self->parse_line_with_internal_parser($decoded);
        }
...
method parse_header ($header, $argument, $mime, $encoding) {
    ...
    if ($header =~ /^Subject$/i) {
        ...
        if ($subject eq '') {
            ...
            $argument = $self->parse_line_with_internal_parser($argument)
