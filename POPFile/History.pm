...
method commit_history {
    ...
    my $update_history = $self->db()->prepare(
...
method get_slot_from_hash ($hash) {
    my $h = $self->db()->prepare( 'select id from history where hash = ? limit 1;' );
    $h->execute( $hash );
