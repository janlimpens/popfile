...
#
# ----------------------------------------------------------------------------
sub global_config_($self, $name, $value=undef)
{
    return $self->module_config_( 'GLOBAL', $name, $value=undef)
}
...
sub setchildexit($self, $value=undef) {
    if (defined $value) {
...
sub pipeready($self, $value=undef) {
    if (defined $value) {
